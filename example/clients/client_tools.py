...
class SuperSpinBox(QtGui.QLineEdit):
    ...
    def step(self, up):
        ...
        cursorPosition = self.cursorPosition() # position, from left, of cursor
        decimalPosition = len(text.split('.')[0]) +1 #position, from left, of decimal
        relPosition = decimalPosition - cursorPosition # how far is the cursor from the decimal point
        if cursorPosition < decimalPosition: # cursor immedeately on either side of decimal is equivalent
            relPosition -= 1
        
        value = float(text)*self.display_factor # what is this in units of display factor = 1?
        step_size = 10.**relPosition*self.display_factor # step by one in the digit to the left of cursor
