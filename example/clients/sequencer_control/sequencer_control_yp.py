...
from yesrgang.okfpga.sequencer.analog_ramps import RampMaker
...
class LoadSaveRun(QtGui.QWidget):
    ...
    def populate(self):
        ...
        self.startnumber = QtGui.QSpinBox()
        ...
        self.layout.addWidget(self.startnumber)
...
class Sequencer(QtGui.QWidget):
    ...
    def connectWidgets(self):
        ...
        self.loadSaveRun.startnumber.valueChanged.connect(self.changeStartNumber)
    ...
    def changeStartNumber(self):
        self.startnumber = self.loadSaveRun.startnumber.value()
    ...
    def mode2bool(self,mode):
        if mode == 'manual':
            return True
        elif mode == 'auto':
            return False
    def bool2mode(self,modeBool):
        if modeBool:
            return 'manual'
        else:
            return 'auto'
    ...
    def onDigitalNameClick(self, channel_name):
        ...
        ttl_name = self.get_name(channel_name)
        ...
        def odnc():
            if QtGui.qApp.mouseButtons() & QtCore.Qt.RightButton:
                ...
                # modearray = yield pserver.get_state(ttl_name)
                # modeManual = modearray[0]
                # mode = self.bool2mode(modeManual)
                server = yield self.cxn.get_server(self.digital_servername)
                mode = yield server.channel_mode(channel_name)
                if mode == 'manual':
                    print 'should be manual:', mode
                    print 'should be True:', mode == 'manual'
                    yield server.channel_mode(channel_name, 'auto')
                    yield pserver.switch_auto(ttl_name)
                    print 'switch %s to auto' %ttl_name
                    state = yield pserver.get_state(ttl_name)
                    print '(ismanual,manualstate,manualinv,autoinv)', state
                else:
                    print 'should be auto:', mode
                    print 'should be false:',mode == 'manual'
                    yield server.channel_mode(channel_name, 'manual')
                    yield pserver.switch_manual(ttl_name)
                    print 'switch %s to manual' % ttl_name
                ...
                pserver = yield self.cxn.get_server('pulser')
                yield pserver.switch_manual(ttl_name, not state)
                print '%s manual switch is = %r'% (ttl_name,state==0)
        return odnc
    ...
    def get_sequence_paramaters(self):
        conductor = yield self.cxn.get_server(self.conductor_servername)
        sp = yield conductor.set_parameters()
    ...
    def runPulserSeq(self,c, sequence):
        ...
        cserver = yield self.cxn.get_server(self.conductor_servername)
        sequence = yield cserver.evaluate_sequence_parameters(s)
        print type(sequence)
        sequence = json.loads(sequence)
        ...
        yield pserver.start_number(self.startnumber)
